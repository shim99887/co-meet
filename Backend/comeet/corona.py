        ...
        for i in range(1, 10):
            for j in range(1, 10):
                for k in range(1, 10):
                    ...
                    weight_1 = {string: (i + 1) * firstWeight for i,
                                string in enumerate(list_1)}

                    # 첫번째 달에서 두번째 달 넘어가는 걸로 소트해서 등수(12 => 1)
                    point_2 = fpop_rate.sort_values(
                        by=['first_popl_rate'], axis=0)
                    list_2 = point_2['gugun'].to_list()
                    weight_2 = {string: (i + 1) * secondWeight for i,
                                string in enumerate(list_2)}

                    # 두번째 달 넘어가는 거에서 세번째 달 넘어가는 걸로 소트해서 등수(1 => 2)
                    point_3 = fpop_rate.sort_values(
                        by=['second_popl_rate'], axis=0)
                    list_3 = point_3['gugun'].to_list()
                    weight_3 = {string: (i + 1) * thirdWeight for i,
                                string in enumerate(list_3)}

                    total_score = Counter(weight_1) + \
                        Counter(weight_2) + Counter(weight_3)
                    ...
                    #     # 계산을 해서 저장
                    #     fpopl_weight.save()
                    print("해당 구의 유동인구 / 전체 유동인구에 대한 가중치 : ", firstWeight)
                    print("첫번째 달에서 두번째 달 유동인구 변화에 대한 가중치 : ", secondWeight)
                    print("두번째 달에서 세번째 달 유동인구 변화에 대한 가중치 : ", thirdWeight)
                    print(total_score)
